...
namespace RhythmEngine.Examples
{
    ...
    public class SongEditor : MonoBehaviour
    {
        ...
        [Space]
        [SerializeField] private bool AutoPlaceResponse = true;
        ...
        private void Awake()
        {
            ...
            int beatCount = (int)barCount * SongToEdit.BeatsPerBar * SongToEdit.BeatSubdivisions;
            _rawBeatCount = rawBarCount * SongToEdit.BeatsPerBar * SongToEdit.BeatSubdivisions;
            _maxBeat = beatCount;
        ...
        private void LoadNotesFromSong(int beatCount)
        {
            ...
            foreach (var note in SongToEdit.CallNotes)
            {
                var gridPosition = TimeToGridPosition(note.Time, 0);
                CreateNewNote(gridPosition);
            }
            foreach (var note in SongToEdit.ResponseNotes)
            {
                var gridPosition = TimeToGridPosition(note.Time, 1);
                CreateNewNote(gridPosition);
            }
        }
        ...
        public void ToggleNote(Vector2Int gridPosition)
        {
            ...
            // Edit corresponding response as well
            if (AutoPlaceResponse && lane == 0) {
                var responseBeat = beat + SongToEdit.CallResponseInterval * SongToEdit.BeatSubdivisions;
                note = _noteGrid[0, beat];
                var responseNote = _noteGrid[1, responseBeat];
                if (note == null && responseNote != null)
                {
                    Destroy(responseNote.gameObject);
                    _noteGrid[lane, responseBeat] = null;
                }
                else if (note != null && responseNote == null)
                {
                    CreateNewNote(new Vector2Int(1, responseBeat));
                }
            }
        ...
        public void Save()
        {
            ...
            SongEditorSongSaver.Save(notes, SongToEdit);
        }
    }
}
